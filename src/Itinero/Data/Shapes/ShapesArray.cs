...
namespace Itinero.Data.Shapes
{
    ...
    public class ShapesArray : ArrayBase<ShapeBase>
    {
        ...
        private readonly ArrayBase<double> _coordinates; // holds the coordinates.
        ...
        /// A new shape index.
        /// </summary>
        public ShapesArray(long size = 1024)
        {
            _index = new MemoryArray<ulong>(size);
            _coordinates = new MemoryArray<double>(size * 2 * ESTIMATED_SIZE);
            _createElevation = (s) =>
            {
                var elevation = new MemoryArray<short>(s);
            ...
            {
                _coordinates[i] = double.MinValue;
            }
        }
        ...
        /// A new shape index.
        /// </summary>
        public ShapesArray(MemoryMap map, long size = 1024)
        {
            _index = new Array<ulong>(map, size);
            _coordinates = new Array<double>(map, size * 2 * ESTIMATED_SIZE);
            _createElevation = (s) =>
            {
        ...
        /// A new shape index.
        /// </summary>
        private ShapesArray(ArrayBase<ulong> index, ArrayBase<double> coordinates, ArrayBase<short> elevation)
        {
            _index = index;
        ...
        public static ShapesArray CreateFrom(Stream stream, bool copy, out long size, bool hasElevation = false)
        {
            ...
            { // just create arrays and read the data.
                ...
                size += indexLength * 8;
                coordinates = new MemoryArray<double>(coordinatesLength);
                size += coordinatesLength * 8;
                coordinates.CopyFrom(stream);
                if (hasElevation)
                {
                    elevation = new MemoryArray<short>(coordinatesLength / 2);
            ...
            { // create accessors over the exact part of the stream that represents vertices/edges.
                ...
                var map2 = new MemoryMapStream(new CappedStream(stream, position + indexLength * 8, 
                    coordinatesLength * 8));
                coordinates = new Array<double>(map2.CreateDouble(coordinatesLength));
                size += coordinatesLength * 8;
                if (hasElevation)
                {
                    var map3 = new MemoryMapStream(new CappedStream(stream, position + indexLength * 8 + coordinatesLength * 8,
                        coordinatesLength));
