    ...
    internal partial class NetworkTile {
        ...
        internal void AddTurnCosts(VertexId vertex, uint turnCostType,
            ...
            while (enumerator.MoveNext()) {
                ...
                if (enumerator.Forward && enumerator.Tail == order) {
                    continue;
                }

                if (!enumerator.Forward && enumerator.Head == order) {
                    continue;
                }

                // set order, it's different.
                SetFromOrder(enumerator, order.Value);
            }
            ...
            while (true) {
                ...
                for (var x = 0; x < max; x++) {
                    for (var y = 0; y < max; y++) {
                        ...
                        if (cost != 0) {
                            yield return (turnCostType, cost);
                        }
                    }
                }
        ...
        private void SetFromOrder(NetworkTileEnumerator enumerator, byte order) {
            ...
            byte? head = null;
            byte? tail = null;
            GetTailHeadOrder(pointer, ref tail, ref head);
