...
public class LuaProfile : Profile
{
    ...
    private LuaProfile(Script script)
    {
        _script = script;
        this.Name = _script.Globals.Get("name").String;
        _hasTurnFactor = _script.Globals.Get("turn_cost_factor").Type == DataType.Function;
        if (!_hasTurnFactor)
        {
    ...
    public override EdgeFactor Factor(IEnumerable<(string key, string value)> attributes)
    {
        lock (_lock)
        {
            var attributesTable = new Table(_script);
            var resultTable = new Table(_script);
            foreach (var (k, v) in attributes)
            {
                attributesTable[k] = v;
            }

            _script.Call(_script.Globals.Get("factor"), attributesTable, resultTable);
    ...
    public override TurnCostFactor TurnCostFactor(IEnumerable<(string key, string value)> attributes)
    {
        ...
        lock (_lock)
        {
            ...
            _script.Call(_script.Globals.Get("turn_cost_factor"), attributesTable, resultTable);
