...
internal sealed class UnsignedNodeIndex
{
    ...
    public bool IsCoreNode(long id)
    {
        ...
        {
            var tempId = this.GetId(_previousIndex + 1);
            if (tempId == id)
            {
                if (this.IsCoreNodeAtIndex(_previousIndex + 1, id))
                {
                    return true;
                }
        ...
        {
            _previousIndex = idx;
            if (this.IsCoreNodeAtIndex(idx, id))
            {
                return true;
            }
    ...
    public bool TryGetValue(long id, out float latitude, out float longitude, out bool isCore)
    {
        ...
        if (!this.GetLatLon(idx, out latitude, out longitude))
        {
            latitude = float.MaxValue;
            longitude = float.MaxValue;
            isCore = false;
    ...
    public long TryGetIndex(long id)
    {
        ...
        {
            var previousId = this.GetId(_previousIndex);
            var offset = 1;
            var nextId = this.GetId(_previousIndex + offset);
            while (nextId == previousId &&
                   _previousIndex + offset + 1 < _idx)
            {
                offset++;
                nextId = this.GetId(_previousIndex + offset);
            }
        ...
        long bottom = 0;
        var top = _idx - 1;
        var bottomId = this.GetId(bottom);
        if (id == bottomId)
        {
        ...
        while (top - bottom > 1)
        {
            ...
            var middleId = this.GetId(middle);
            if (middleId == id)
            {
                while (middle - 1 > 0 &&
                       this.GetId(middle - 1) == id)
                {
                    middle--;
                }
