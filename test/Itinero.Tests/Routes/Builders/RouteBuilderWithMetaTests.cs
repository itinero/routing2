    ...
    public class RouteBuilderWithMetaTests
    {
        ...
        public void RouteBuilder_OneEdgeWithMeta_MetaIsContained()
        {
            var (routerDb, vertices, edges) = RouterDbScaffolding.BuildRouterDb(new (double longitude, double latitude)[]
                ...
                new (int @from, int to, IEnumerable<(double longitude, double latitude)>? shape, List<(string, string)> attributes)[]
                {
                    (0, 1, new (double longitude, double latitude)[0], new List<(string, string)>{("name","Straatnaam")})
                });
