...
public class DijkstraTurnCostTests
{
    ...
    public async Task Dijkstra_OneToOne_TwoHopsShortest_WithoutTurnCost_ShouldFindTwoHopPath()
    {
        ...
        Assert.Equal(ushort.MaxValue, result.path.Offset2);
        using var enumerator = result.path.GetEnumerator();
    ...
    [Fact]
    public async Task Dijkstra_OneToOne_TwoHopsShortest_OnlyWithTurnCost_ShouldFindTwoHopPath()
    {
        var routerDb = new RouterDb();
        EdgeId edge1, edge2, edge3;
        VertexId vertex1, vertex2, vertex3;
